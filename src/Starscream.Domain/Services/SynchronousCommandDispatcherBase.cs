    ...
    public abstract class SynchronousCommandDispatcherBase : ICommandDispatcher
    {
        public void Dispatch(IUserSession userSession, object command)
        {
            ...
            foreach (object handler in matchingCommandHandlers)
            {
                ...
                {
                    //in a command queue, any exception would not affect this web server,
                    //so, to simulate a command queue and prevent us from depending on
                    //exception behavior generated by command handlers, this "catch" 
